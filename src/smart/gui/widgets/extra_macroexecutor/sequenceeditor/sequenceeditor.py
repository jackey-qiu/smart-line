...
class TaurusSequencerWidget(TaurusWidget):
    ...
    def onPlaySequence(self):
        ...
        if (doorState == PyTango.DevState.ON or
            ...
            #all ones with a pause being checked will be ruled out
            #total_rows = self.tree.model().rowCount()
            exclude_ids = []
            new_ids = []
            parent = self.tree.selectionModel().currentIndex().parent()
            for i,each in enumerate(ids):
                if not self.tree.model().nodeFromIndex(self.tree.model().index(i,3,parent))._pause:
                    new_ids.append(each)
                else:
                    exclude_ids.append(each)
            if len(new_ids)==0:
                return
            ...
            tree = etree.fromstring(self.tree.toXmlString(pretty=True))
            for id in exclude_ids:
                for pause in tree.xpath(f"//macro[@id='{id}']"):
                    tree.remove(pause)
            door.runMacro(etree.tostring(tree, encoding='unicode', pretty_print = False))
        elif doorState == PyTango.DevState.STANDBY:
