...
class motorMeter(GraphicsLayoutWidget, TaurusBaseComponent):
    ...
    def update_motor_viewer(self):
        ...
        motor_values = self.parent.settings_object["Motors"].values()
        motor_names = self.parent.settings_object["Motors"].keys()
        models = [key + "/Position" for key in motor_values]
        self._setModel_list(list(motor_names), models)
    ...
    def _setModel_list(self, motor_name_list, motor_model_list):
        ...
        for i, motor in enumerate(motor_name_list):
            ...
            self.motor_pos_marker_list.append(
                pg.InfiniteLine(
                    angle=90,
                    movable=True,
                    pen=pg.mkPen(colors[i], width=2, style=QtCore.Qt.SolidLine),
                )
            )
            ...
            self.motor_pos_marker_list[-1].sigPositionChangeFinished.connect(
                lambda state, motor=motor, line=self.motor_pos_marker_list[
                    -1
                ]: self.move_motor_to_finishing_line(motor, line)
            )
        ...
        try:
            for i in range(len(self.motor_name_list)):
                if evt_src is self.getModelObj(key=(TaurusBaseComponent.MLIST, i)):
                    ...
                    self.text_label_list[i].setText(
                        f"{self.motor_name_list[i]}={round(float(evt_value.rvalue.m),3)}"
                    )
                    break
    ...
    def move_motor_to_finishing_line(self, motor_name, infinitline_obj):
        ...
        if self.parent._qDoor != None:
            ...
            self.text_label_list[self.motor_name_list.index(motor_name)].setText(
                f"{motor_name}={round(float(infinitline_obj.value()),3)})"
            )
            self.parent._qDoor.runMacro(
