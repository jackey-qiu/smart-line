...
class PlotManager(Qt.QObject, TaurusBaseComponent):
    ...
    def handleEvent(self, evt_src, evt_type, evt_value):
        try:
            if evt_src is self.getModelObj(key="motor"):
                self.setValue(float(evt_value.rvalue.m))
        except Exception as e:
            self.info("Skipping event. Reason: %s", e)

    def setValue(self, value):
        if self.bind_obj != None:
            self.bind_obj.setValue(value)
