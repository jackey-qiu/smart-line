...
class _ZeroMQ(object):
    ...
    def _topic_filter(self, topic):
        """
        Performs filtering on the topic to check if it should be accepted.
        ...
        if "" in self._subscription_list:
            return True

        # Default filter of ZMQ.
        elif self._topic_filter_mode == "startswith":
            return True

        # Strict topic filter.
        elif self._topic_filter_mode == "strict":
            return topic in self._subscription_list
...
class ClientSub(_ZeroMQ):
    def __init__(
        ...
        topic_filter : str, optional
            The topic filter mode.
            Default is strict.
            Options are: strict, startswith
        ...
        self._topic_filter_mode = topic_filter
...
def connection_inverter(conn_mode, conn_type):
    """
    Inverts the connection mode and type.
    ...
    -------
    str
        The inverted connection mode.

    str
        The inverted connection type.
    """
...
def connection_selector(
    ...
    **kwargs
    ...
    if conn_mode == 'client':
        ...
        raise ValueError("Unsupported connection mode. ({})".format(conn_mode))
